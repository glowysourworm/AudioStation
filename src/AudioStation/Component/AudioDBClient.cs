...
namespace AudioStation.Component
{
    ...
    public class AudioDBClient : IAudioDBClient
    {
        ...
        {
            return Task.Run<AudioDBNowPlayingViewModel>(() =>
            {

                var artist = AudioDB.AudioAPI.SearchArtistById(artistId);
                var album = AudioDB.AudioAPI.SearchAlbumById(albumId);
        ...
        public Task<AudioDBArtistViewModel> SearchArtist(string artistName)
        {
            ...
            {
                var artist = AudioDB.AudioAPI.SearchArtist(artistName);
                long artistId = 0;

                if (artist != null && long.TryParse(artist.idArtist, out artistId))
                {
                    var albums = AudioDB.AudioAPI.SearchAlbumsByArtistId(artistId);
                    if (albums != null)
                    {
                        // Map Albums
